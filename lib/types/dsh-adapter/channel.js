...
import { createUserMessage, isTokenDelta, MessageId, ReasoningEffortId, } from '@deepseek-ai/dsh-llm';
...
import { clearResumeTarget, forgetSession, readLastUsed, readResumeTarget, touchSession, writeResumeTarget } from '../sessionHistory.js';
    ...
    const state = {
        ...
        async rewindTo(row) {
            ...
            try {
                ...
                state.notify(`Session rewound, but workspace attachment failed · ${error instanceof Error ? error.message : String(error)}`, { color: 'warning', timeoutMs: 8000 });
            }
        ...
        async resumeTo(sessionId) {
            ...
            try {
                ...
                state.notify(`Session resumed, but workspace attachment failed · ${error instanceof Error ? error.message : String(error)}`, { color: 'warning', timeoutMs: 8000 });
            }
        ...
        async newSession() {
            ...
            try {
                ...
                state.notify(`Session created, but workspace attachment failed · ${error instanceof Error ? error.message : String(error)}`, { color: 'warning', timeoutMs: 8000 });
            }
            streaming = undefined;
            reasoning = undefined;
            toolCards.clear();
        ...
        async switchModel(provider, model) {
            ...
            try {
                ...
                state.notify(`Model switched, but workspace attachment failed · ${error instanceof Error ? error.message : String(error)}`, { color: 'warning', timeoutMs: 8000 });
            }
            streaming = undefined;
            reasoning = undefined;
            toolCards.clear();
        ...
        async listSessions() {
            ...
            try {
                ...
                const local = headers.filter(header => sessionCwdMatches(state.cwd, header.cwd ?? ''));
                ...
                for (const record of records.slice(0, SESSION_TITLE_DEPTH)) {
                    ...
                    if (info === undefined)
                        continue; // keep the basename fallback
                    ...
                    if (info.title !== undefined)
                        record.title = shortenTitle(info.title);
                }
    ...
    const bindAgent = () => {
        ...
        agentSubscriptions = [
            ...
            agent.ctx.on('system-prompt/assemble', async (_assembly, _context, next) => {
                ...
                if (assembled.variables.model !== undefined && assembled.variables.provider !== undefined)
                    return assembled;
 ...
 */
export async function expandMentions(fs, cwd, text) {
